...
function renderFilters() {
  ...
  document.querySelector('#filters').innerHTML = categories.map(category => `<button class="filter ${category === state.category ? 'active' : ''}" type="button" data-category="${category}">${category === 'ALL' ? 'All' : category}</button>`).join('');
...
function renderMenu() {
  ...
  document.querySelector('#menu-grid').innerHTML = foods.map(food => `<article class="food-card"><div class="food-image" style="background-image:url('${food.imageUrl}')"></div><div class="food-content"><h3>${food.name}</h3><p>${food.description}</p><div class="food-footer"><strong>${money(food.price)}</strong><button class="add-button" type="button" data-add="${food.id}" aria-label="Add ${food.name} to cart">+</button></div></div></article>`).join('');
...
function changeQuantity(foodId, amount) { const quantity = (state.cart.get(foodId) || 0) + amount; quantity > 0 ? state.cart.set(foodId, quantity) : state.cart.delete(foodId); renderCart(); }

function renderCart() {
  const entries = [...state.cart.entries()];
  document.querySelector('#cart-count').textContent = entries.reduce((sum, [, quantity]) => sum + quantity, 0);
  if (!entries.length) { document.querySelector('#cart-items').innerHTML = '<p class="muted">Your cart is waiting for something tasty.</p>'; document.querySelector('#cart-total').textContent = '$0.00'; return; }
  let total = 0;
  document.querySelector('#cart-items').innerHTML = entries.map(([foodId, quantity]) => { const food = state.foods.find(item => item.id === foodId); total += food.price * quantity; return `<div class="cart-item"><div><strong>${food.name}</strong><small>${money(food.price)} each</small></div><div class="quantity"><button type="button" data-change="-1" data-id="${foodId}">-</button><span>${quantity}</span><button type="button" data-change="1" data-id="${foodId}">+</button></div><button class="remove" type="button" data-remove="${foodId}">Remove</button></div>`; }).join('');
...
document.querySelector('#order-form').addEventListener('submit', async event => {
  ...
  if (!state.cart.size) { message.textContent = 'Add at least one item before placing your order.'; return; }
  ...
  try {
    ...
    state.cart.clear(); renderCart(); event.target.reset(); message.textContent = '';
...
loadMenu().catch(error => { document.querySelector('#menu-grid').innerHTML = `<p class="muted">${error.message}. Please refresh the page.</p>`; });
